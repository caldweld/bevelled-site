...
export default function PortfolioPage() {
  async function Content() {
    ...
    const { data: rows, error } = await supabase
      ...
      .select('title, slug, summary, cover_url, images:portfolio_images(url)')
      .order('created_at', { ascending: false })
    if (error) {
      throw new Error(`PORTFOLIO_QUERY_FAILED: ${error.message}`)
    }
    if (!rows || rows.length === 0) {
      throw new Error('PORTFOLIO_EMPTY: Add items in Supabase (portfolio_items & portfolio_images)')
    }
    const items = rows.map((r: any) => ({
      title: r.title,
      slug: r.slug,
      summary: r.summary,
      images: (r.images || []).map((i: any) => i.url) || (r.cover_url ? [r.cover_url] : []),
    }))
    return (
